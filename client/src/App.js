...
class App extends Component {
  state = { loaded: false, kycAddress: "", tokenSaleAddress: "", userToken: 0};
  ...
  componentDidMount = async () => {
    try {
      ...
      this.setState({ loaded: true, tokenSaleAddress: MyTokenSale.networks[this.networkId].address}, this.updateUserToken);
  ...
  handleKeyWhitelisting = async () => {
    ...
    console.log(await this.kycInstance.methods.allowed(this.state.kycAddress).call());
    alert("KYC for "+this.state.kycAddress+ " is finished");
  }
  ...
  render() {
    ...
    return (
      <div className="App">
        <h1>WhitelistDAO Token Sale</h1>
        <p>Get Your Tokens Today</p>
        <h2>Kyc Whitelisting</h2>
        Address to allow: <input type="text" name="kycAddress" value={ this.state.kycAddress } onChange={ this.handleInputChange }/>
        <button type="button" onClick={ this.handleKeyWhitelisting }>Add to Whitelist</button>
        <h2>Buy Tokens</h2>
        <p>If you want to buy tokens, send Wei to this address: { this.state.tokenSaleAddress }</p>
        <p>You currently have: { this.state.userToken } tokens</p>
